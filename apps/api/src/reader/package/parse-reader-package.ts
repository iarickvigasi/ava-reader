...
import {
  normalizeReaderPackageMetadata,
...
// the metadata normalization the reader depends on.
export function parseReaderPackage(buffer: Buffer): ReaderPackage {
  ...
  return normalizeReaderPackageMetadata(raw as ReaderPackageWithLegacyAuthors);
}
