...
export function DeleteBookCard({ libraryItemId }: { libraryItemId: string }) {
  ...
  return <>
    <ActionCard danger title={t("title")} description={t(online ? "description" : "offline")}
      icon={TrashBoldIcon} disabled={!online || !isLoaded || !isSignedIn || pending}
      onClick={() => { setError(false); setOpen(true); }} />
    {open && <ModalShell maxWidth="md" labelledBy={titleId} onClose={() => { if (!busy.current) setOpen(false); }}>
      <div className="rounded-modal bg-surface p-6 shadow-(--shadow-card)">
        <h2 id={titleId} className="font-reader text-2xl text-title">{t("title")}</h2>
        <p className="mt-4 text-copy">{t("body")}</p>
        {error && <p role="alert" className="mt-3 text-danger">{t("error")}</p>}
        <div className="mt-5 flex justify-end gap-2">
          <Button variant="soft" disabled={pending} onClick={() => setOpen(false)}>{t("cancel")}</Button>
          <Button variant="danger" disabled={pending || !online} onClick={() => void confirm()}>{t(pending ? "deleting" : "confirm")}</Button>
        </div>
      </div>
    </ModalShell>}
  </>;
