...
import { MoreVerticalIcon } from "./ai-chats-icons";
...
export function CreateChatButton({ onClick }: { onClick?: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex h-10 w-full items-center justify-center rounded-lg border border-ink bg-brand-fill px-3 font-(--font-ui) text-[0.78rem] uppercase tracking-[0.16em] text-brand-foreground transition hover:bg-brand-fill-strong"
    >
...
export function RowActionsMenuTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex size-7 shrink-0 items-center justify-center rounded-full text-ink/55 transition",
        "hover:bg-soft-tone-fill hover:text-ink",
        "opacity-0 group-hover/ai-chat-row:opacity-100 focus:opacity-100",
