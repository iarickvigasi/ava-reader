...
export function selectionUnitAt(
  ...
  let first = selected;
  let last = selected;
  if (isWordGrapheme(graphemes[selected].segment)) {
  ...
  // select one visible symbol, including next to non-wordwise scripts.
...
function isWordGrapheme(segment: string): boolean {
  return !DELIMITER.test(segment) && !SYMBOL_SCRIPT.test(segment) && !EMOJI.test(segment);
}
