...
> [[4.5-route-precaching]], [[4.1-offline-reading]], [[4.4-cache-priming]] ·
...
*behavior* contract lives in [[4.5-route-precaching]] (and 6/11); this spec never restates it.
...
3. networkFirstDoc is the only network-first strategy: the RSC variant was removed with the RSC
   cache ([[4.5-route-precaching]] §5).
4. Comment policy: rationale lives in specs, not the file. A regression already documented in
   [[4.5-route-precaching]] (redirect pass-through, prefetch-stub poisoning, shell-key fallback,
   precache redirect:"manual") gets a one-liner naming the failure + spec pointer, e.g.
   `// Redirects pass through — substituting cache swallows the Clerk handshake (spec 4.5 §6).`
...
- [x] No comment block restates rationale already in [[4.5-route-precaching]]; every trimmed trap
