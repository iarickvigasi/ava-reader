...
6. **Progress chip:** the page/reader header shows a non-interactive pill with two states, reported
   ...
   reserves its chip height, so going offline/priming never reflows the header or shifts reader
   content): offline → Offline chip wins; else **Slow chip** (degraded-but-online connection, see
