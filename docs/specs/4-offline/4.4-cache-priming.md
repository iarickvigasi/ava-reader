...
> Status: active · Updated: 2026-09-04 · ADRs: [[3-offline-first-dexie-buckets]] · Related:
> [[4.2-save-sync]], [[4.5-route-precaching]], [[4.10-slow-connection]],
> [[6-reading-sessions-progress]] · Code:
> apps/web/features/offline/{prime,status}, apps/web/features/offline/buckets/progress,
> apps/web/components/app/core/{header-status-chip,caching-indicator,slow-connection-indicator}.tsx,
> apps/api/src/reader (GET reader/progress)
...
## Behaviour (tiers)
1. **Metadata (always when online):** home (incl. stats), the library list, **every collection in
   full** (smart *and* custom — the list payload previews only 4 books each), per-book info for all
   smart-collection books, and user **preferences** (`GET /api/me/preferences`). Skipped only on
   slow-2g/2g. A complete pass then **prunes** `libraryItems` to the union of ids it saw — the only
   moment absence is knowable ([[4-offline/_overview]], Write paths) — never touching a row with an
   unsynced `offlineRequestedDirty` toggle. Custom shelves add order, not books, so the book-info
   fan-out is unchanged.
2. **Content (offline-marked books + current book):** for each book with `offlineRequested`
   ([[4.2-save-sync]]) and the current continue-reading book (the home resume target), not yet
   cached locally — save chapters + cover (`saveBookOffline`, sequential), plus its **highlights**
   (`/annotations`), **AI comments** (`/ai-comments`), and **reading progress** (`GET
...
6. **Progress chip:** the page/reader header shows a non-interactive pill with two states, reported
   to a client store (no SW): - **Content** — "Caching for offline access {done}/{total} books"
   while offline-marked content downloads (`done` = books handled, `total` = offline-marked books).
   Skipped (no pill) when nothing is marked offline. - **Ready** — the honest "safe to read offline"
   cue: shown only once **all three offline substrates are confirmed present** — metadata, the route
   shells ([[4.5-route-precaching]], via the SW ack), and the current book's content — held ~5s
   (`DWELL_MS` in `use-header-chip`), then hidden. It must **never** appear while a static route
   would fail offline or the current book would hit the missing-book modal. With no current book
   (fresh user) metadata + shells suffice. First completion only (a warm device early-returns);
   re-shown after a reconcile download finishes. The pill owns the one header status slot (which
   reserves both width and height, including while idle, so status changes never move header
   controls or shift reader content). Mobile app headers center AVA between equal-width side
   columns. Compact pills show numeric progress or a short localized Ready label, collapsing to
   their status icon when space is tight; full labels remain available to assistive technology
   and in tooltips. Status priority: offline → Offline chip wins; else **Slow chip** (degraded-but-online connection, see
   [[4.10-slow-connection]]) wins over priming; else content → ready → nothing. A run that ends
   without completing (offline mid-pass, blocked on Save-Data consent) clears the pill rather than
...
[[6-reading-sessions-progress]]). The offline reader reads its resume position from the progress
bucket ([[2-reader/2.5-resume]]).

## Edge cases
**Cold-load kick vs. auth timing:** the kick edge (`wasOnline`, in `reduceKick`) advances only once
Clerk is ready (`isLoaded && isSignedIn`). `isLoaded` is false on the first render(s), so advancing
there would consume the first-online transition before the auth gate passes — the kick would be lost
and the primer would run only on a later offline→online flip, which is why book-info details never
cached on a normal online load. While not ready the edge holds; the first ready render is the
transition. Offline mid-pass / page closed → done-flag unset, resumes. Save-Data ON + declined →
content skipped this run, re-offered next session. Library fetch fails → no enumeration, retry (and
no prune — an incomplete pass may never delete). Quota floor → terminal + warning toast. User opens
a book mid-prime → that save wins. **Metadata never goes clean** (one book's `/api/library/[slug]`
persistently fails — slug mismatch, endpoint bug) → `prime:metadata:doneAt` is never set, so the
primer re-fetches every book-info each load and that book's details never cache, surfacing as the
offline route fallback on its details page. **Save-Data ON + content declined** → the current book
isn't cached, so the Ready cue is withheld rather than over-promising; metadata + shells still
prime. **Current book in the evictable auto-slot** → priming leaves it, but opening another book can
evict it (spec 4.1), so the next pass re-caches whatever the current book then is. **Current book's
background save fails transiently** (Clerk token not booted during the idle pass, a network blip) →
the content tier is best-effort and still marks completion, but the Ready cue is **withheld**:
...
modal. The next reconcile re-attempts the save and emits the cue once content lands — enforced here,
not merely promised by the chip, which trusts the beat.

## Acceptance criteria
- [ ] Metadata for all smart-collection books — plus user preferences — primes on first home load
  (even under Save-Data).
- [ ] A complete pass caches every library book; a `/app/library` visit never reduces it.
...
- [ ] A book primed on a new device opens at the last-read page offline (not chapter 1). On
  ...
  separately via the progress sync runner ([[6-reading-sessions-progress]]).
- [ ] During content priming the header shows "Caching for offline access n/m books" climbing to
  completion; it yields to the Offline chip when offline and the Slow chip on a degraded connection
  ([[4.10-slow-connection]]), and on a warm (completed) device appears only during a reconcile.
