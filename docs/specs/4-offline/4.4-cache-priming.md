...
2. **Content (offline-marked books + current book):** for each book with `offlineRequested`
   ([[4.2-save-sync]]) and the current continue-reading book (the home resume target), not
...
6. **Progress chip:** the page/reader header shows a non-interactive pill with two states, reported
   ...
   - **Ready** — the honest "safe to read offline" cue: shown only once **all three offline
     substrates are confirmed present** — metadata, the route shells ([[4.5-route-precaching]], via
...
the evictable auto-slot** → priming leaves it, but opening a different book can evict it (spec 4.1),
