...
> [[4.2-save-sync]], [[4.5-route-precaching]], [[6-reading-sessions-progress]] · Code:
...
6. **Progress chip:** the page/reader header shows a non-interactive pill with two states, reported
   ...
   content): offline → Offline chip wins; else content → ready → nothing. A run that ends without
   completing (offline mid-pass, blocked on Save-Data consent) clears the pill rather than leaving a
   stuck count.
