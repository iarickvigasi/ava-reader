...
**Cover read path.** `BookCover` always tries the network cover URL first (so an online repaint
never pays for a Dexie read); only once that fails does it — given a `libraryItemId` — read the
saved `coverBlob` (`readCoverBlob`/`useCoverBlobUrl`, buckets/book) and swap in an object URL. No
`libraryItemId` (an anonymous catalog card, e.g. the home listening/featured sections) or no saved
blob (the book isn't offline-saved, or its cover save failed) falls through to
`BookCoverFallback` — never the browser's native broken-image icon. A "failure" also covers the
case where the image finishes (0×0) with no `error` event at all: offline, that often happens
before hydration attaches the listener, so the mount-time `complete` check on the `<img>` ref
catches it too.
...
yet), so a network cover failure there falls straight to `BookCoverFallback` even for a book that is
in fact saved offline, instead of trying its blob first.
