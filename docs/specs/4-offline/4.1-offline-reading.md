...
> [[4.2-save-sync]], [[4.3-save-button]] · Code:
...
3. Explicit saves are sticky; the evictable auto-cache is dropped when the user opens **another**
   ...
   cached, every _other_ `savedAutomatically && !savedOffline && !offlineRequested` book is deleted.
   Synced offline intent protects content even when the reader's auto-save wins on reconnect.
   Opening an already-saved book evicts the previous auto-cache too (not only a fresh auto-save),
   and a stale
...
**Cover read path — cache-first, lazy write-through.** Given a `libraryItemId`, `BookCover` checks
the Dexie-cached `coverBlob` (`readCoverBlob`/`useCoverBlobUrl`, buckets/book) before the network —
a cover never changes, so once cached there's no reason to ever refetch it, offline-saved or not.
While that cache check is unresolved, the frame holds `BookCoverFallback` rather than starting a
network request the cache may make moot. No `libraryItemId` (an anonymous catalog card, e.g. the
home listening/featured sections — never cached, matching the non-goal above) or a cache miss falls
through to the network `src`; a successful network view then writes the bytes through to Dexie in
the background (`persistCoverFromNetwork`) so the _next_ view — even offline — is a cache hit. This
is opportunistic, not prefetch: nothing downloads until a cover is actually viewed once. A source
failing — cache or network — swaps to the other; both failing (or no `src` at all) falls to
`BookCoverFallback`, never the browser's native broken-image icon. A "failure" also covers the case
where an image finishes (0×0) with no `error` event at all: offline, that often happens before
hydration attaches the listener, so the mount-time `complete` check on the `<img>` ref catches it
too.
...
bearing: `libraryItems` is a preview projection that a saved book routinely drops out of, while
`books` is the authority on what is downloaded ([[4-offline/_overview]], Shared data model). Resolving from
`libraryItems` alone made fully-downloaded books report "This page needs a connection".

## Edge cases

Storage quota exceeded → toast + stop; interrupted download → resume; eviction while reading the
evicted book; cover missing (save never ran, or the cover step of a save failed while chapters
succeeded — content still counts as saved) → `BookCoverFallback` placeholder, not a broken image.
Offline never evicts: opening another book while disconnected keeps the
...
**iOS storage cap (known limitation, not fixable in code).** iOS/iPadOS Safari deletes _all_
...
- [ ] An offline-requested book cached by auto-save survives eviction when another book opens;
      clearing intent makes it evictable again unless `savedOffline` still protects it.
...
Other screens still read `libraryItems` as if it were the whole library ([[4-offline/_overview]], Shared data
model). The reader no longer does; each remaining surface needs its own evidence before it changes.
Home's listening and featured-catalog sections render `BookCover` without a `libraryItemId` (their
payload types carry no library-item id — the latter is often a catalog entry the user hasn't added
yet), so they never check or populate the Dexie cache at all — a network failure there falls
straight to `BookCoverFallback` even for a book that is in fact in the library.
