...
> [[3-offline-first-dexie-buckets]] · Related: [[7-auth]], [[4.2-save-sync]] · Code:
...
- In: a per-user Dexie DB; a root reconciler that preserves data on expiry and clears on explicit sign-out and on a different-user login;
...
2. On explicit app sign-out (not session expiry or userId → null): `await` a full wipe — delete the current user's DB, the
   `ava-reader-sw-*` caches, and every user-scoped localStorage key + cookie. Then reset in-memory
   buckets.
3. On a different user resolving (account switch, or cold-start as B): `await` purge of all _other_
...
Purge/clear is best-effort and retried on the next load (isolation holds even if it fails). Account transitions gate and remount account-owned views before rendering the new account.
...
Session expiry and remote revocation preserve local ownership and pending changes; server sync
requires the same authenticated account. See [[6-durable-offline-authentication]].
