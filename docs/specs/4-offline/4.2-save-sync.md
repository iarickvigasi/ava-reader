...
`item-row.ts` (`mergeListPayloadItemRow`); the book-info write keeps its own in
`write-book-info.ts`, because that payload does carry `offlineRequested` and so falls back
