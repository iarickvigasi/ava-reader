...
> [[4.1-offline-reading]], [[4.4-cache-priming]], [[4.3-save-button]],
> [[4.8-offline-books-collection]] · Code: apps/api/src/library,
...
- The button derives "Download queued" from Dexie (`offlineRequested && !hasBookContent`), so
  re-opening the page shows the real state, not idle ([[4.3-save-button]]).
- The primer's reconcile pass ([[4.4-cache-priming]]) fetches the content on the next reconnect / app
...
collection ([[4.8-offline-books-collection]]).
...
Known gap: the intent survives two different merge rules. List payloads go through
`collections/item-row.ts` (`mergeListPayloadItemRow`); the book-info write keeps its own in
`book-info/write-book-info.ts`, because that payload does carry `offlineRequested` and so falls back
differently (payload → prior, and `offlineRequestedBaseline` → prior baseline). Unifying them
would change what a book-info revalidation writes, so they stay separate until a case forces it.
