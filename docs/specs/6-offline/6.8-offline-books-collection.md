...
> [[7-library/_overview]], [[6.2-save-sync]], [[6.3-save-button]] · Code:
...
- Non-goals: per-device "what's actually cached here" (that stays the book-info card's job,
  [[6.3-save-button]]), manual add/remove of members, storage-budget UI.
...
2. Membership = `offlineRequested` ([[6.2-save-sync]]), so it is the same set on every
...
([[6.4-cache-priming]]). Download failed or storage floor hit → listed without content;
