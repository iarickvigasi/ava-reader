...
> [[6.2-save-sync]], [[6.3-save-button]] · Code:
...
`books` is the authority on what is downloaded ([[6-offline/_overview]], Shared data model). Resolving from
...
## Known gaps
Other screens still read `libraryItems` as if it were the whole library ([[6-offline/_overview]], Shared data
