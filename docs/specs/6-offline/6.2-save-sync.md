...
> [[6.1-offline-reading]], [[6.4-cache-priming]], [[6.3-save-button]],
> [[6.8-offline-books-collection]] · Code: apps/api/src/library,
...
- The button derives "Download queued" from Dexie (`offlineRequested && !hasBookContent`), so
  re-opening the page shows the real state, not idle ([[6.3-save-button]]).
- The primer's reconcile pass ([[6.4-cache-priming]]) fetches the content on the next reconnect / app
...
collection ([[6.8-offline-books-collection]]).
