...
> [[6.1-offline-reading]], [[6.2-save-sync]] · Code:
...
- `offlineRequested` (+ `offlineRequestedDirty`) — server-synced intent ([[6.2-save-sync]]);
...
download then runs unattended via the primer's reconcile pass on reconnect ([[6.4-cache-priming]]),
