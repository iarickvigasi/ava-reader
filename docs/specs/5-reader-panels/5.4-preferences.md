...
5. The daily reading goal accepts whole minutes from 1 to 1440. Blur or Enter saves a valid
   edit; Escape restores the saved value. Invalid drafts show a localized error and leave the
   saved preference intact. The default is 60 minutes when no goal has been saved.
6. Saving a goal updates home progress immediately, including offline. All seven displayed
   days are compared with the current goal; recorded reading time is unchanged.
...
- [ ] Goal edits save on blur/Enter, reject invalid values, and cancel on Escape.
- [ ] Offline goal edits survive reopen and sync on reconnect, including edits made during a save.
