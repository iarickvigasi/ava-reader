...
> Status: shipped · Updated: 2026-09-04 · ADRs: [[2-openrouter-and-byo-key]] · Code:
> apps/web/components/app/reader/overlays/ai-toolbox, apps/api/src/ai-comments,
> apps/web/features/offline/buckets/ai-comments/sync.ts
...
4. Opened offline (or while a queued request is still pending) → the panel shows a "waiting for
   ...
   when the queued request replays on reconnect. The same happens on a technically-online but
   unresponsive connection: the generate request's `fetch()` is guarded by a 5s time-to-first-byte
   timeout (headers only — once the response starts, the streaming read loop is never cut off, so a
   slow-but-progressing generation completes normally). If no response starts within 5s, the request
   is aborted and treated exactly like a network drop — the shared retry/backoff loop
   (`sync-core.ts`) reschedules it, and the placeholder row is patched back to `status: "queued"` (it
   had flipped to `"streaming"` the moment the attempt started) so the panel shows the same "waiting
   for connection" copy rather than an indefinite "Generating…" animation. This status reset applies
   to every retry outcome from the same call site, not just the timeout — a plain network error
   (e.g. the fetch throws immediately) gets the same honest "queued" placeholder instead of getting
   stuck on "Generating…" until the next reload.
...
(delete drops still toast). A response that starts within 5s but then stalls mid-stream (idle
connection, not TTFB) is not covered by the timeout — it keeps waiting on that read indefinitely,
same as before this change; only a genuine mid-stream drop (the existing read-loop `catch`) triggers
a retry, and that gap is deliberate, not an oversight (see Open questions). Abort mid-stream; missing
API/credit key (see ADR 3). Context
...
- [ ] A tool opened offline shows a localized "waiting for connection" placeholder, not a blank
  panel.
- [ ] A generate request that gets no response within 5s is aborted and queued for retry — the
  panel shows the "waiting for connection" placeholder, not an indefinite "Generating…" animation,
  and the result appears automatically once a retry succeeds.
- [ ] A generate request that does get a response within 5s is never aborted mid-stream by the
  timeout, however long the stream itself takes.
...
## Open questions
Per-user key/credit enforcement; rate limiting; tool result versioning. A mid-stream idle stall (past
TTFB) has no timeout today — only a genuine connection drop retries; worth an idle-between-chunks
timeout if this proves to be a real occurrence, not just the TTFB hang.
