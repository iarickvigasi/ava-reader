...
- Not yet on prod: the schema collapses to a single version; no data-preserving migration is
  written.
- Does not by itself keep `/api/me` PII out of cached SSR shells — clearing the SW cache on
  switch/sign-out covers it for now; baking PII out of shells is separate hardening.
