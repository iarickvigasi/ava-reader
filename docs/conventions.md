...
boundaries, continuing blockquotes with `> ` and list items with a hanging indent. Exceptions:
...
- Tailwind v4 + cn() (lib/cn.ts). Theme via data-theme + CSS vars; no hardcoded colors. Visual
  language (tokens, radius, borders, buttons, modals) lives in [styles.md](styles.md).
- No React Query/SWR: server components fetch the initial payload (server-api.ts); clients subscribe
  to buckets. Exception (ADR 4): reader/book-info/collection pages are generic shells — a client
